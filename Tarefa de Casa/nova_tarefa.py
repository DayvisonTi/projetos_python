...
def main():
    ...
    if not os.path.exists(arquivo):
        with open(arquivo, 'w') as file:
            yaml.dump({'tarefas':None}, file)
    else:
        with open(arquivo, 'r') as file:
            lista = yaml.safe_load(arquivo, file)
    ...
    with open(arquivo, 'a', encoding='utf-8') as file:
        yaml.dump(tarefas, file, default_flow_style=False, sort_keys=True, allow_unicode=False)
...
def listar_tarefas(tarefas, lista):
    if tarefas:
